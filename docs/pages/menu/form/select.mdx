...
`select.☘-select[data-primary]` 这个属性需要启用后才可以继承主题和拥有某些局部样式，或置于 `☘-input-wrapper[data-primary]` 中

局部变量 `--color-select` 可以直接设置主题颜色RGB值， 这优先于 `--color-input-wrapper`

<T tag="select" class="☘-select" data-primary="success" style="width:180px">
    <option value="genshin">原神</option>
    <option value="hsr">崩坏：星穹铁道</option>
    <option value="zzz">绝区零</option>
</T>
...
```pug
select.☘-select(data-primary="success" style="width:180px")
    option(value="genshin") 原神
    option(value="hsr") 崩坏：星穹铁道
    option(value="zzz") 绝区零
```
...
<T tag="select" class="☘-select" data-primary style="width:180px">
    <option value="genshin">原神</option>
    <option value="hsr">崩坏：星穹铁道</option>
    <option value="zzz" data-primary="success">绝区零</option>
</T>

```pug
select.☘-select(data-primary style="width:180px")
    option(value="genshin") 原神
    option(value="hsr") 崩坏：星穹铁道
...
<T tag="select" class="☘-select" disabled style="width:180px">
    <option value="genshin">原神</option>
    <option value="hsr">崩坏：星穹铁道</option>
    <option value="zzz" selected>绝区零</option>
</T>

```pug
select.☘-select(disabled style="width:180px")
    option(value="genshin") 原神
    option(value="hsr") 崩坏：星穹铁道
...
- `--padding-horizontal-select` 横向内边距
- `--padding-vertical-select` 纵向内边距
...
<T tag="label" class="☘-form-item">
    <span>故障机型</span>
    <T class="☘-input-wrapper" data-primary>
        <T tag="select" name="type" class="☘-select" style="width:180px">
            <button>
...
label.☘-form-item
    ...
    .☘-input-wrapper(data-primary)
        select.☘-select(name="type" style="width:180px")
            button
                selectedcontent
                svg(
