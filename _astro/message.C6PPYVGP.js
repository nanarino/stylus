...
`);let u=null;for(const c of m){const s=c.match(/^(\s+)\S+/);if(s){const d=s[1].length;u?u=Math.min(u,d):u=d}}if(u!==null){const c=u;i=m.map(s=>s[0]===" "||s[0]==="	"?s.slice(c):s).join(`
...
`)),i}}function v(r){return new Promise(e=>setTimeout(e,r))}const y=(r,e)=>r?l`
        ...
        `,g={"--background-color-message":"var(--primary-5)","--box-shadow-color":"var(--primary-4)"};class x{queue;constructor(e=document.body){this.reset(e)}reset(e=document.body){this.queue?.remove(),this.queue=document.createElement("div"),this.queue.className=`${f}-message-queue`,e.append(this.queue)}async emit(e){const t=document.createElement("div");let n=3e3,o="",p={},a=e??"☘";if(typeof a!="string"&&(n=a.duration??3e3,o=`${a.primary||""}`,p=a.style??{},a=`${a.content??"☘"}`),t.innerHTML=l`
            <div class="${f}-message">
        ...
        `,o){const i=t.firstElementChild;i.dataset.primary=o,Object.entries(p).map(m=>i.style.setProperty(...m))}this.queue.appendChild(t),t.style.height=`${t.offsetHeight}px`,t.style.transition="opacity 1s, height 2s",await v(n),t.style.opacity="0",t.style.height="0",await v(2e3),this.queue.removeChild(t)}async info(e,t="info"){return await this.emit({content:l`
            ...
            `})}async success(e){return await this.emit({content:l`
                ${y(!0,"check")}
                <span>${e}</span>
            `,primary:"success",style:g})}async danger(e){return await this.emit({content:l`
                ${y(!0,"close")}
                <span>${e}</span>
            `,primary:"danger",style:g})}get error(){return this.danger}async warning(e){return await this.emit({content:l`
                ${y(!0,"exclamation")}
                <span>${e}</span>
            `,primary:"warning",style:g})}}const q=new x;export{q as _,l as d,y as i};
